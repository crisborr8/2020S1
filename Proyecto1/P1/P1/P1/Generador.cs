        ...
        public void Generar()
        {
            if (verificarConjuntos())
            {
                ...
                for (int i = 0; i < OperacionCerradura.Count; i++)
                {
                    ...
                    Tabla_Cerradura(OperacionCerradura[i].getS1());
                    codigo += "</table>>];\n}";
        ...
        private void AFD()
        {
            ...
            for (int t = 0; t < TablaCerradura.Count; t++)
            {
                ...
                foreach (int v in visitados)
                {
                    if (v != -1)
                    {
                        codigo += TablaCerradura[t].Letra + " -> " + TablaCerradura[t].Ir[v].Letra_C +
                                " [label=\"" + TablaCerradura[t].Ir[v].Letra_I + "\"];\n";
                        letra = TablaCerradura[t].Ir[v].Letra_C;
                    }
                }
                if (t == TablaCerradura.Count - 1)
                    codigo += letra + " [shape=doublecircle];\n";
            }
        }
        ...
        private Automata getNodo(Automata actual, int id)
        {
            ...
            if (actual.getS2() != null)
            {
                if (!visitados.Contains(actual.getS2().getN()))
                    if (actual.getS2().getN() <= id) return getNodo(actual.getS2(), id);
            }
